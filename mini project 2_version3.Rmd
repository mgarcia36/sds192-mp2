...
top_ten_cmte <- contributions_by_alaska %>%
  group_by(cmte_name) %>%
  tally(sort = TRUE)
```
...
top_10 <- subset(contributions_by_alaska, cmte_name %in% top_ten_cmte$cmte_name[0:10])

ggplot(top_10, aes(cmte_name, transaction_amt)) +
  geom_bar(stat = "identity") 
```
