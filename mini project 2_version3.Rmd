...
contributns_by_alaska <- contributions_by_state(state_arg = "AK")

contributions_by_florida <- contributions_by_state(state_arg = "FL")

contributns_by_alaska <- contributions_by_state(state_arg = "AK")
ggplot(contributns_by_alaska, aes(cmte_name, transaction_amt)) +
  ggtitle("Committees that Received Contributions Made By Individuals in 2011") +
  geom_bar(stat = "identity", fill = "olivedrab")
...
contributions_by_florida <- individuals %>%
  mutate(transaction_date = mdy(transaction_dt)) %>%
  filter(state == "FL", 
         entity_type == "IND",
         transaction_date >= "2011-01-01",
...
tallying_florida <- contributions_by_florida %>%
  group_by(cmte_name) %>%
  tally(sort = TRUE)
