...
#individuals_to_can <- candidates %>%
#inner_join(individuals, by = c("cmte_id"))
...
#pipes individuals data into the new variable ind_cont
#creates new variable transaction date into the data frame
#select() selects specific columns from data frame
#filter() filters rows that have CA as the state 

ind_cont <- individuals %>%
...
entity_data <- contributions_new(data = contributions, state_arg = "CA", entity_type_arg = "CCM")
glimpse(entity_data)
...
#+1 for annotating your data wrangling pipeline 
#pipes contributions data into the new variable cont
#filters out the entity type chosen by the user, or the hard coded example "CCM"
#also filtered by giving a range of transaction_amt being from $0-$5000 and narrowing the data set down to California
#select() choses specific columns from data frame such as transaction_type,entity_type,state,and transaction_amt
#ggplot creates a histogram specifying that the x axis is based off of the tranaction_amt variable with 15 bins of monetary ammounts for donations.

cont <- contributions %>%
  mutate(transaction_date = mdy(transaction_dt)) %>%
  filter(transaction_date >= "2011-01-01" & transaction_date <= "2011-12-31",
         entity_type == "CCM",
         transaction_amt >= 0 & transaction_amt <= 5000, 
         state == "CA") %>%
...
cont_2 <- cont %>%
  summarise(n = n_distinct(transaction_type))
...
ggplot(cont, aes(x = transaction_amt)) + 
  ggtitle("Amount of contributors per donation amount") +

  geom_histogram(bins = 11, color = "black", fill = "olivedrab") +
  theme_hc() +
  theme(axis.text.x = element_text(color = "black", 
                           size = 12, family = "Trebuchet MS"),
  ...
  scale_x_continuous(name = "Donation Amount",
                     expand = c(0,0),
                     limits = c(0, 6000),
                     labels = c("0" = "",
                                "1000" = "$1000",
